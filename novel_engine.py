...
# 목표를 여유 있게 잡아, 실제 결과가 5,000자 밑으로 떨어지지 않도록 한다.
MIN_CHARS = 5500
MAX_CHARS = 6200
...
def _episode_system_prompt(bible: dict) -> str:
    return (
        ...
        "[문피아 연재 원칙 — 반드시 지킬 것]\n"
        f"1. 분량(가장 중요): 공백 포함 {MIN_CHARS}~{MAX_CHARS}자. "
        f"{MIN_CHARS}자에 못 미치면 절대 끝내지 말고, 장면 묘사·인물의 심리·감각적 디테일·대화의 결을 "
        "더 촘촘히 채워 분량을 반드시 확보하라. 대사가 많은 화는 글자 수가 적게 나오기 쉬우니, "
        "대사 사이사이에 행동·표정·공간 묘사를 넣어 분량을 늘려라. 사건을 더 벌이라는 뜻이 아니라, "
        "정해진 사건을 더 밀도 있게 그리라는 뜻이다.\n"
